	...
	<MainLayout>
		<div class="">
			...
			<template v-if="$page.props.auth.user.type == 'admin'">
				<div
					v-if="form.changes_approved == '1'"
					class="alert alert-success d-block"
				>
					Customer has approved, you can complete shopping
				</div>
				<div
					v-else-if="form.changes_approved == '0'"
					class="alert alert-warning d-block"
				>
					Customer has not approved the order please wait until customer
					approves
				</div>
			</template>
			...
			>
				<li
					class="nav-item"
					role="presentation"
					v-show="form.form_type == 'shopping'"
				>
					<button
							...
							>
								<div class="row">
									...
									<div class="col-md-2" v-show="this.form.store_id !== ''">
										<div class="form-group">
											<breeze-label
												for="pickup_charges"
												value="Pickup Charges (USD)"
											/>
											<input
												type="text"
												name="pickup_charges"
												id="pickup_charges"
												v-model="form.pickup_charges"
												readonly
											/>
										</div>
									</div>
								...
								<div class="row mb-2">
									<div class="col-md-8">
										<input
											type="radio"
											@change="getGrandTotal(1)"
											v-model="form.pickup_type"
											name="pickup_type"
											value="pickup_only"
											:required="setRequired('tab2')"
											:checked="form.pickup_type == 'pickup_only'"
										/>
										Only Pickup
										<input
											type="radio"
											...
											:required="setRequired('tab2')"
											:checked="form.pickup_type == 'shipping_xps_purchase'"
										/>
                        ...
                        <input type="radio" v-model="form.shipping_xps" name="shipping_xps" value="0" :required="setRequired('tab2')"> No
									...
									<div class="col">
										<div class="form-group">
											...
											<input
												...
												name="shipping_from_shop"
												@keyup="getGrandTotal"
												id="shipping_from_shop"
												type="number"
												step="any"
												min="0"
												class="form-control"
							...
							<div class="row">
								<div class="col-md-12">
									...
									<div class="row mt-3">
										...
										<div class="col-md-2" v-show="form.form_type == 'shopping'">
											<breeze-label
												class="d-none d-md-flex"
												for="url"
												value="Url"
										...
										<div
											class="col-md"
											v-show="form.pickup_type != 'pickup_only'"
										>
											<breeze-label
												class="d-none d-md-flex"
												for="price"
									...
									>
										...
										>
											<div class="form-group">
												...
												<input
													...
													placeholder="URL"
													:required="form.form_type === 'shopping'"
												/>
											</div>
										</div>
										<div
										...
										>
											<div class="form-group">
												...
												<input
													v-model="item.price"
													v-on:change="addTax($event)"
													v-on:keyup="addTax($event)"
													v-on:click="addTax($event)"
													:min="0"
													ref="price"
													name="price"
													step="0.01"
										...
										>
											<div class="form-group">
												...
												<input
													...
													id="price_with_tax"
													type="number"
													class="form-control price_with_tax"
													placeholder="PWT"
													required
													readonly
												/>
									...
									<div
										class="row mb-2"
										v-show="
											form.shipping_from_shop != null &&
											form.shipping_from_shop != 0
										"
									>
										<div class="col-6 col-md-1 offset-md-9">
											<breeze-label
												class="float-md-end"
												for="shipping_from_shop"
												value="Shipping From Shop"
											/>
										</div>
										<div class="col-6 col-md-1 p-0">
											<input
									...
									>
										...
										<div class="col-6 col-md-1 p-0">
											<input
												...
												@keyup="adminServicesCharges()"
												placeholder="charges"
												:min="0"
												required
												:readonly="$page.props.auth.user.type == 'customer'"
											/>
										</div>
									</div>
									<!-- Box Price -->
									<div
										class="row mb-2"
										v-show="this.form.form_type !== 'shopping'"
									>
										<div class="col-6 col-md-2 offset-md-8">
											<breeze-label
												class="float-md-end"
							...
							<div class="order-button">
								...
								>
									...
									>
										<a
											class="btn mt-2 btn-primary ml-2"
											v-on:click="approveChanges()"
											>Approve & Checkout</a
										>
									</template>
									<template
								...
								<template v-else-if="$page.props.auth.user.type == 'customer'">
									<p>
										Note: Your shopping has been rejected! Contact customer care
								...
								>
									...
									<a
										...
										v-on:click="changeAdminStatus(order.id, 'approved')"
										class="ml-2 me-2 btn mt-2 btn-success"
										>Approve</a
									>
									<a
										v-if="order.status != 'rejected'"
										v-on:click="changeAdminStatus(order.id, 'rejected')"
										class="ml-2 me-2 btn mt-2 btn-danger"
										>Reject</a
									>
								</template>
								...
								>
									...
									>
										<a
											v-on:click="changeToCompleteShopping()"
											class="btn mt-2 btn-primary float-md-end"
										>
								...
								>
									<a
										class="btn mt-2 btn-primary float-md-end disabled"
		...
		<div class="container">
			<div>
				<div class="row">
					...
					<div class="col-12">
						<label class="btn btn-primary float-right" @click="saveComment()"
							>Add Comment</label
						>
					</div>
				</div>
			...
			<fieldset
				...
				class="border p-4 mb-4 col-12 mt-2"
			>
				...
				>
					...
					<div class="card-footer" style="padding: 5px 10px">
						<p class="float-right">{{ comment.user.name }}</p>
					</div>
				</div>
			</fieldset>
		</div>
	...
	export default {
		...
		methods: {
			...
			filterStores() {
				...
				axios
					.get("/shop-for-me/filter-stores/" + this.form.warehouse_id)
					.then(({ data }) => {
			...
			addTax(event) {
				...
				this.form.items[index].sub_total = parseFloat(
					net_total * quantity
				).toFixed(2);
				...
				if (this.form.pickup_type == "pickup_only") {
					...
					if (e == 1 && this.form.is_service_charges != 1) {
						...
						if (serviceCharges <= 5 && serviceCharges > 0.01) {
							serviceCharges = this.form.service_charges = 5.0;
						} else {
					...
					pickup_charges =
						this.form.form_type === "shopping" ? 0 : pickup_charges;
					additionalCharges =
						this.form.form_type === "shopping" ? 0 : additionalCharges;
					var total =
						parseFloat(sum) +
						parseFloat(charges) +
						parseFloat(pickup_charges) +
